...
Сценарий: Я проверяю ТЧ_ОбъектыГруппы "НеСодержитНКО"
	*По умолчанию
...
//TODO привести работу элементов функционала Основание решения к одному виду
// за основу надо взять решение в ДивидендвхВыплатах
Сценарий: Я проверяю элементы формы функционала Основание решения
	Если элемент формы с именем 'СпособЗаполненияРеквизитовПротоколаРешения' стал равен '' Тогда
		И у группы страниц "ГруппаДнныеОснования" текущая страница "ГруппаДнныеОснованияНет"
	Иначе
		И у группы страниц "ГруппаДнныеОснования" текущая страница "ГруппаДнныеОснованияЕсть"
		Если элемент формы с именем 'СпособЗаполненияРеквизитовПротоколаРешения' стал равен 'Заполнить по данным корпоративного мероприятия' Тогда
			И у группы страниц "ГруппаКорпоративноеМероприятие" текущая страница "ГруппаРеквизитыАвтоматом"
		ИначеЕсли элемент формы с именем 'СпособЗаполненияРеквизитовПротоколаРешения' стал равен 'Заполнить реквизиты руками' Тогда
			И у группы страниц "ГруппаКорпоративноеМероприятие" текущая страница "ГруппаРеквизитыРуками"

Сценарий: Я проверяю элементы формы функционала Основание решения ДивидендыВыплаты
	//TODO
